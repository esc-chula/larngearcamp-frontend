...
const StatusInfo: React.FC<StatusInfoProps> = ({ profileStatus }) => {
  ...
  const { picture, parentalConsent, transcript } = application?.documentStateDetails
  const failedDocuments = [
    { id: 1, ...picture },
    { id: 2, ...parentalConsent },
    { id: 3, ...transcript }
  ].filter(detail => detail.status === "EMPTY" || detail.status === "CHANGE_REQUIRED")
  return (
    <Paper elevation={1} className={classes.paper}>
      <Typography variant="h5" className={classes.title} dangerouslySetInnerHTML={{ __html: statusInfoConstant[profileStatus].title }} />
      ...
      {failedDocuments.map((detail, index) => (
        <>
          ...
          <Typography
            ...
            color="primary"
            className={`${classes.content} ${classes.marginLeft}`}
            dangerouslySetInnerHTML={{ __html: detail.comment }}
          />
        </>
      ))}
